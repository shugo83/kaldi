...
# default gpu=1
# option gpu=1 -p general
# option gpu=* -p gpu --gres=gpu:1 --time 4:0:0  # this has to be figured out
...
default gpu=1
option gpu=1 -p general
option gpu=* -p gpu --gres=gpu:1 --time $0  # this has to be figured out
# note: the --max-jobs-run option is supported as a special case
# by slurm.pl and you don't have to handle it in the config file.
EOF
...
if ($ret != 0) {
  if ($sync && $ret == 256) { # this is the exit status when a job failed (bad exit status)
    if (defined $jobname) { $logfile =~ s/\$SLURM_ARRAY_TASK_ID
	y/*/g; }
